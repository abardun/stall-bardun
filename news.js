// Bardun Stall AB — News Feed
// Auto-updated daily by news-update.js. Do not edit manually.
window.BARDUN_NEWS = {
  updated: "2026-04-26",
  articles: [
    {
        "title": "Ungt och gammalt på Mantorp",
        "url": "https://www.mantorphastsportarena.se/nyheter/ungt-och-gammalt-pa-mantorp",
        "source": "mantorphastsportarena.se",
        "date": "2026-04-15",
        "excerpt": "Inför torsdagens tävlingar på Mantorp lyfts Jonathan Bardun fram med Metarie, som jagar sin tredje seger tillsammans med sin tränare och kusk på hemmabanan."
    },
    {
        "title": "Floripa tog första segern i karriären – har sprungit in mer än 100 000",
        "url": "https://www.travronden.se/referat/floripa-tog-forsta-segern-i-karriaren-har-sprungit-in-mer-an-100-000",
        "source": "travronden.se",
        "date": "2026-03-10",
        "excerpt": "Floripa (e. Father Patrick) tog karriärens första seger tillsammans med sin tränare Jonathan Bardun."
    },
    {
        "title": "V64 Tips Eskilstuna 9/1 – Metarie med Bardun i V64-4",
        "url": "https://www.rekatochklart.com/trav/v64-tips/v64-tips-eskilstuna-9-1/",
        "source": "rekatochklart.com",
        "date": "2026-01-09",
        "excerpt": "Travanalys inför V64 på Eskilstuna där Metarie, tränad och körd av Jonathan Bardun, pekas ut som intressant i V64-4. Bardun väntas vara offensiv bakom Metarie om hästen klarar det snäva startspåret från 20 meters tillägg."
    },
    {
        "title": "Superjackpot i V85®: Barduns häst Blyger S.R.P. siktar mot stjärnorna",
        "url": "https://nastagangare.se/superjackpot-i-v85-barduns-hast-blyger-s-r-p-siktar-mot-stjarnorna/",
        "source": "nastagangare.se",
        "date": "2025-12-17",
        "excerpt": "Med fyra segrar på sex försök har Blyger S.R.P. imponerat stort under Jonathan Barduns ledning. En superjackpot på 60 miljoner kronor väntar i V85-loppen i Halmstad."
    },
    {
        "title": "Trippel för Jonathan Bardun",
        "url": "https://www.mantorphastsportarena.se/nyhetsarkiv/nyheter/2025/november/trippel-for-jonathan-bardun/",
        "source": "mantorphastsportarena.se",
        "date": "2025-11-24",
        "excerpt": "Den 26-årige tränaren Jonathan Bardun vann tre lopp på en dag på Mantorp Hästsportarena med hästarna Coraline, Metarie och Blyger S.R.P. Metarie, inköpt för 210 000 kr på Travera-auktionen, vann sitt debutlopp på hemmabanan efter bara tre veckors lätt träning hos stallet."
    }
]
};
